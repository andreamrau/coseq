...
of RNA-seq data), using transformed profiles rather than the raw counts directly. The package implements mixture models and the K-means algorithm, performs 
parameter estimation using an expectation-maximization algorithm, and performs model selection using either the Integrated Completed Likelihood (ICL) criterion or the slope heuristics approach. The methods implemented in this package are described in detail in the following three publications:
...
* Godichon-Baggioni, A., Maugis-Rabusseau, C. and Rau, A. (2017) Clustering transformed compositional data using K-means, with applications in gene expression and bike sharing system data. Manuscript in preparation.
...
```{r}
runArcsin <- coseq(counts, K=2:25, model="Normal", transformation="arcsin", meanFilterCutoff=200)
runLogit <- coseq(counts, K=2:25, model="Normal", transformation="logit", meanFilterCutoff=200, 
                  verbose=FALSE)
```
